...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password !== confirm) {
      setRes("Les mots de passe ne correspondent pas");
      return;
    }
    setLoad(true);
    try {
      const response = await axios.post(
        linkBackend + "log/reset-password/" + token,
        { password },
      );
      setRes(response.data.message);
      // Si succès on redirige vers le login après 2 secondes
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-[var(--color-gray-light)]" />
              <input
                type="password"
                name="password"
